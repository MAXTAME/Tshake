<?php
define('BOT_TOKEN', '445903035:AAG7mbtRd8CLoXxPFmLELnklm37VUW5Ew_s');
...
function processMessage($message) {
  ...
  if (isset($message['text'])) {
    ...
    if (strpos($text, "/start") === 0) {
      apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "اهلا بك في بوت اللعبة الشهيرة 🌀📤 \n\n  XO 💜 Game \n\n تمتع بلعب اللعبة 🎮 مع اصدقائك بسرعة خيالية 💚 وايضاََ بدون مشاكل🌟 \n\n مع بوت فريق VIP TEAM🎙 \n\n يمكنك متابعة قناة البوت📬 : @Maxtame ", 'reply_markup' => array(
            "inline_keyboard"=>array(
			    array(array("text"=>"تواصل معنا 📤 ","url"=>"https://telegram.me/iq_tW_bot"),
				array("text"=>"جديدنا 📢","url"=>"https://telegram.me/Maxtame")),
			    array(array("text"=>"تحدي اللعبة 💜","switch_inline_query"=>md5(date("YMDms"))))
			)
		)));
    } 
  }
			  ...
			  if($Tab[$i][$j]["text"]!=" ") apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"يمكنك تحديد الزر المطلوب",'show_alert'=>false));
                  ...
                  //				  
				    if(Win($Tab)=="⭕️"||Win($Tab)=="❌") {
					    ...
					    apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"الاعب الاول:$P1Name(❌)\nالاعب الثاني$P2Name(⭕️)\n\nالنتيجه تعادل!","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));  
					    exit;
				    }
						...
						$Tab[3][0]["text"=>"channel" ,'url'=>"t.me/aboodr"];
			     	
						//Tab
						
						$NextTurn=getChat($NextTurn);
				        apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"اللاعب الاول 🌀📤 : $P1Name(❌)\nاللاعب الثاني🎧💜 : $P2Name(⭕️)\n\n الدور للاعب 🌟 : $NextTurn($NextEmoji) \n\n يمكنك متابعة قناة البوت📬 : @Maxtame ","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));
					    exit;
				    }
